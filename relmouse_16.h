...
class USBMouse16 : public USBHID {
  public:
    explicit USBMouse16(bool connect_blocking = true, uint16_t vendor_id = 0x8234,
                        uint16_t product_id = 0x0001, uint16_t product_release = 0x0001);

    explicit USBMouse16(USBPhy *phy, uint16_t vendor_id = 0x8234, uint16_t product_id = 0x0001,
                        uint16_t product_release = 0x0001);

    virtual ~USBMouse16();

    bool update(int16_t x, int16_t y, uint8_t button_bits, int8_t z);
...
USBMouse16::USBMouse16(bool connect_blocking, uint16_t vendor_id, uint16_t product_id,
                       uint16_t product_release)
    : USBHID(connect_blocking, 0, 0, vendor_id, product_id, product_release) {}

USBMouse16::USBMouse16(USBPhy *phy, uint16_t vendor_id, uint16_t product_id,
                       uint16_t product_release)
    : USBHID(get_usb_phy(), 0, 0, vendor_id, product_id, product_release) {}

USBMouse16::~USBMouse16() {}

bool USBMouse16::update(int16_t x, int16_t y, uint8_t button_bits, int8_t z) {
    _mutex.lock();

    HID_REPORT report;
    report.data[0] = button_bits;
...
#define TOTAL_DESCRIPTOR_LENGTH                                                                    \
    ((1 * CONFIGURATION_DESCRIPTOR_LENGTH) + (1 * INTERFACE_DESCRIPTOR_LENGTH) +                   \
     (1 * HID_DESCRIPTOR_LENGTH) + (2 * ENDPOINT_DESCRIPTOR_LENGTH))
...
const uint8_t *USBMouse16::configuration_desc(uint8_t index) {
    ...
    memcpy(_configuration_descriptor, configuration_descriptor_temp,
